...
int hoht_push_hashed(Hoht_Table* table, __m128i hash, void* value);
...
int hoht_get_hashed(Hoht_Table* table, __m128i hash, void* out);
...
void* hoht_get_value_hashed(Hoht_Table* table, __m128i hash);
...
int
hoht_push_hashed(Hoht_Table* table, __m128i hash, void* value) {
    if (table->occupancy_ratio >= table->occupancy_max) hoht_grow(table, 2);

    int index = (int)(MeowU64From(hash, 0) % (uint64_t)table->capacity);
    ...
    if (entry_ptr->flags & HASH_TABLE_OCCUPIED) {
        ...
        if (MeowHashesAreEqual(entry_ptr->hash, hash)) {
            /* same entry */
            return index;
        }
        else {
...
hoht_push(Hoht_Table* table, const char* key, void* value) {
    __m128i hash = MeowHash(MeowDefaultSeed, strlen(key), (void*)key);
    return hoht_push_hashed(table, hash, value);
}

int
hoht_push_length(Hoht_Table* table, const char* key, int key_length, void* value) {
    __m128i hash = MeowHash(MeowDefaultSeed, key_length, (void*)key);
    return hoht_push_hashed(table, hash, value);
}

int
hoht_get_hashed(Hoht_Table* table, __m128i hash, void* out) {
    int index = (int)(MeowU64From(hash, 0) % (uint64_t)table->capacity);
    ...
    if (entry_ptr->flags & HASH_TABLE_OCCUPIED) {
        if (MeowHashesAreEqual(entry_ptr->hash, hash)) {
            ...
            Hoht_Table_Entry* entry = entry_ptr->next;
            while (entry) {
                if (MeowHashesAreEqual(entry->hash, hash)) {
                    if (out) {
...
hoht_get(Hoht_Table* table, const char* key, void* out) {
    return hoht_get_length(table, key, (int)strlen(key), out);
}

void*
hoht_get_value_hashed(Hoht_Table* table, __m128i hash) {
    int index = (int)(MeowU64From(hash, 0) % (uint64_t)table->capacity);
    Hoht_Table_Entry* entry_ptr = (Hoht_Table_Entry*)((char*)table->entries + (sizeof(Hoht_Table_Entry) + table->entry_size_bytes) * index);
    if (entry_ptr->flags & HASH_TABLE_OCCUPIED) {
        if (MeowHashesAreEqual(entry_ptr->hash, hash)) {
...
int hoht_delete_hashed(Hoht_Table* table, __m128i hash) {
    ...
    while (entry && !MeowHashesAreEqual(entry->hash, hash)) {
        prev = entry;
        entry = entry->next;
    }
    if (!entry) {
...
#if 1//defined(HOHT_SERIALIZE_IMPLEMENTATION)
int
