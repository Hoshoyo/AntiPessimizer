...
uint32_t hpa_parse_uint32_length(const char** stream, int length);
uint16_t hpa_parse_uint16_length(const char** stream, int length);
uint8_t  hpa_parse_uint8_length(const char** stream, int length);
...
int      hpa_parse_identifier_length(const char** stream, int length);
...
uint32_t
hpa_parse_uint32_length(const char** stream, int length)
{
	uint32_t result = (uint32_t)hpa_parse_uint64_length(stream, length);
	return (int32_t)result;
}
...
uint16_t
hpa_parse_uint16_length(const char** stream, int length)
{
	uint16_t result = (uint16_t)hpa_parse_uint64_length(stream, length);
	return (uint16_t)result;
}
...
uint8_t
hpa_parse_uint8_length(const char** stream, int length)
{
	uint8_t result = (uint8_t)hpa_parse_uint64_length(stream, length);
	return (uint8_t)result;
}
...
// Parses an identifier until given length. Formed by alphanumeric 
// characters and underlines.
int
hpa_parse_identifier_length(const char** stream, int length)
{
	const char* at = *stream;
	if (length > 0 && at && !hpa_is_number(*at))
	{
		while (length > 0 && (hpa_is_alphanum(*at) || *at == '_'))
		{
			at++;
			length--;
		}
...
// Parses a sequence of whitespace characters, advacing the stream.
